<?php
class CSZSitemap_HtmlGenerator extends Typecho_Widget implements Widget_Interface_Do
{
...
foreach ($tags as $tag) {
    ...
    $tag_contents .= '<a href="'.$tag['permalink'].'" title="'.$tag['name'].'" target="_blank">'.$tag['name'].'</a>'."\n";
}

		$html = str_replace("%tag_contents%",$tag_contents,$html);  // tags
